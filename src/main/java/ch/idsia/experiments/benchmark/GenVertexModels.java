...
public class GenVertexModels {

    static int numDecimals = 5;
    ...
    public static void main(String[] args) throws IOException, ParserConfigurationException, SAXException {
        ...
        for(String bnetFile : getFiles(preciseFolder)) {
    ...
    public static SparseModel buildVmodel(SparseModel bnet, int nVert) {
        ...
        for(int x : bnet.getVariables()) {
            ...
            if(vmodel.getParents(x).length == 0)
                vmodel.setFactor(x, VertexFactor.random(vmodel.getDomain(x), nVert, numDecimals, false));
            else
